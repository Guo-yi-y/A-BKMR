...
a_kmbayes = function (y, Z, X = NULL, iter = 1000, family = "gaussian", id = NULL,
  ...
  if (!is.null(control.params)){
    ...
    bkmr:::validateControlParams(varsel, family, id, control.params)
  } else {
  ...
  if (is.null(starting.values)) {
    ...
    bkmr:::validateStartingValues(varsel, y, X, Z, starting.values, rmethod)
  }
  if (family == "gaussian") {
...
a_OverallRiskSummaries = function (fit, y = NULL, Z = NULL, X = NULL, qs = seq(0.25, 0.75,
  ...
  risk_overall = bind_rows(lapply(qs, function(quant){
    ...
    return(data.frame(quant = quant, est = diff, sd = diff.sd))
  }))
...
a_SingVarRiskSummaries = function (fit, y = NULL, Z = NULL, X = NULL, data.comps, which.z = 1:ncol(Z),
  ...
  df = bind_rows(lapply(1:nrow(para), function(i){
    ...
    diff <- drop(cc %*% preds$postmean)
    diff.sd <- drop(sqrt(cc %*% preds$postvar %*% cc))
    return( data.frame(q.fixed = q.fixed[para$qf[i]], variable = z.names[para$wz[i]], est = diff, sd = diff.sd))
  }))
...
#' @param data.comps a list including objects used for approximate gaussian process, which is generated by function "a_kmbayes"
#' @export
a_SingVarIntSummaries = function (fit, y = NULL, Z = NULL, X = NULL, data.comps, which.z = 1:ncol(Z),
  ...
  if (is.null(z.names))
    z.names <- paste0("z", 1:ncol(Z))
  ints <- sapply(which.z, function(whichz) a_SingVarIntSummary(whichz = whichz,
                                                               fit = fit, Z = Z, X = X, y = y, data.comps = data.comps, qs.diff = qs.diff, qs.fixed = qs.fixed,
                                                               method, sel = sel, ...))
