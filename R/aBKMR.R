                     ...
                     Zgen = "norm", ind = 1:2, family = "gaussian")
{
  ...
  HFun1 <- function(z, ind = 1) {
    4 * plogis(z[ind[1]], 0, 0.3)
  }
  HFun2 <- function(z, ind = 1:2) {
    1/4 * (z[ind[1]] + z[ind[2]] + 1/2 * z[ind[1]] * z[ind[2]])
  }
...
#' @param point1 reference point used to plot the joint exposure-response curve
#'
                                        ...
                                        sel = NULL, data.comps, point1 = NULL)
{
  ...
  preds <- a_ComputePostmeanHnew(fit = fit, y = y,
...
#' @param method method for obtaining posterior summaries at a vector of new points. Options are "approx" and "exact"; defaults to "approx", which is faster particularly for large datasets; see details
#' @param sel logical expression indicating samples to keep; defaults to keeping the second half of all samples
#' @param point1 reference point used to plot univariate exposure-response curves
#' @export
...
#' @param X a numeric vector or matrix of predictor variable(s) on which yhat is regressed
#' @export
...
#' @importFrom stats median plogis rbinom
NULL
